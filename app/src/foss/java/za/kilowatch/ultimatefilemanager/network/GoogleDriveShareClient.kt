...
/**
 * FOSS build stub for GoogleDriveShareClient.
 *
 * Google Drive is not available in the FOSS build — it requires proprietary
 * Google Cloud OAuth credentials. This stub keeps shared-code branches compiling.
 * These branches are unreachable at runtime because the Google Drive option is absent
 * from the AddOnlineStorageActivity FOSS override.
 */
object GoogleDriveShareClient {

    private fun unsupported(): Nothing =
        throw UnsupportedOperationException("Google Drive is not available in the FOSS build.")
    ...
    suspend fun uploadStream(
        ...
        totalSize: Long, onProgress: ((Long, Long) -> Unit)? = null
    ) { unsupported() }
