        ...
        override fun run() {
            ...
            while (!isInterrupted) {
                ...
                if (blockedDuration >= WATCHDOG_TIMEOUT_MS) {
                    ...
                    // 55. The main thread is sampled inside the framework's per-glyph
                    //     text-advance measurement while the Text Viewer measures its
                    //     wrap-content content EditText during a normal window-measure
                    //     pass — top frame `android.graphics.Paint.
                    //     nGetRunCharacterAdvance`/`getRunCharacterAdvance` (the native
                    //     per-character advance computation, the innermost frame of the
                    //     framework's text measurement) under `TextLine.getRunAdvance`/
                    //     `handleText`/`handleRun`/`measureRun`/`measure`/`metrics` ->
                    //     `Layout.getLineExtent`/`getLineMax` -> `TextView.desired` ->
                    //     `TextView.onMeasure`, measuring the content EditText
                    //     (`activity_text_viewer.xml` / `activity_text_viewer_tv.xml`:
                    //     ScrollView -> HorizontalScrollView -> LinearLayout ->
                    //     EditText, `wrap_content` width), reached from a frame-draw
                    //     traversal measure pass (`Choreographer.doFrame` ->
                    //     `ViewRootImpl.doTraversal` -> `performTraversals` ->
                    //     `measureHierarchy` -> `performMeasure`) — reported from a
                    //     Hisilicon SMART_TV, SDK 34, app 1.8.6-GOOGLE. The text being
                    //     measured is app-bounded: the Text Viewer paginates at 16 KB
                    //     per page (`PAGE_BYTE_SIZE`) and caps edit mode at 128 KB
                    //     (`EDIT_MAX_BYTES`), so a single page's glyph measurement is
                    //     bounded work that cannot by itself occupy the main thread for
                    //     5 s; the main thread is RUNNABLE (a thread parked inside a
                    //     >5 s block cannot be processing a fresh vsync frame at sample
                    //     time), and the stack has zero
                    //     `za.kilowatch.ultimatefilemanager` frames — the only
                    //     non-platform frame is the bundled-library
                    //     `androidx.appcompat.widget.ContentFrameLayout` in the window
                    //     decor chain, which breaks `isPureFrameworkStack`. The >5 s
                    //     block is device-side slowness / CPU starvation on the low-end
                    //     TV (the report's own `DlnaSsdpListener`, `NanoHttpd Main
                    //     Listener`, `pool-2-thread-1` and `DefaultDispatcher-worker-*`
                    //     threads are all RUNNABLE, busy with DLNA/SSDP discovery and
                    //     the HTTP file server, starving the main thread) or a
                    //     post-stall sample. None of the existing text-layout filters
                    //     match: `isRecyclerViewTextLayoutStall` (33) requires a
                    //     RecyclerView frame, `isNativeAllocationRegistryTextLayoutStall`
                    //     (43) requires the `VMRuntime.notifyNativeAllocationsInternal`
                    //     top frame, `isTextMeasureSpanQueryStall` (25) requires a
                    //     `SpannableStringBuilder` top frame, and
                    //     `isTextMeasurementDuringInputStall` (18) requires the IME
                    //     `BaseInputConnection.replaceText` path. The
                    //     `AnrWatchdogThread` now treats a main-thread stack whose top
                    //     frame is `Paint.getRunCharacterAdvance`/`nGetRunCharacterAdvance`,
                    //     with a `TextLine` measurement frame, a `TextView.onMeasure`
                    //     frame, a `HorizontalScrollView` frame and a `ScrollView` frame
                    //     (the Text Viewer's wrap-content-in-horizontal-scroll layout),
                    //     reached from a frame-draw traversal (`Choreographer.doFrame`/
                    ...
                    //     and resets its heartbeat instead of writing a report. Genuine
                    //     freezes keep the main thread inside app business logic — an app
                    //     frame on the stack (e.g. a main-thread `setText` of an
                    //     unbounded document, or a custom view measuring heavy text), a
                    //     top frame that is not the per-glyph advance under the
                    //     TextLine/`TextView.onMeasure` measure chain, a text
                    //     measurement NOT reached from a ScrollView/HorizontalScrollView
                    //     frame-draw traversal (e.g. a RecyclerView row or a
                    //     non-Text-Viewer screen), or a blocking primitive (a lock,
                    //     file/network/database I/O or binder frame) — and are still
                    //     reported.
                    val isTextMeasureWrapContentStall =
                        topFrame?.className == "android.graphics.Paint" &&
                        (topFrame?.methodName == "getRunCharacterAdvance" ||
                         topFrame?.methodName == "nGetRunCharacterAdvance") &&
                        // The framework's per-glyph text measurement chain inside the
                        // TextLine measure path (walks every glyph of the measured line).
                        mainStackTrace.any {
                            it.className == "android.text.TextLine" &&
                            (it.methodName == "getRunAdvance" ||
                        ...
                        // The Text Viewer's wrap-content-in-horizontal-scroll layout:
                        // a TextView with wrap_content width inside a HorizontalScrollView
                        // inside a ScrollView, whose onMeasure must measure the full
                        // (app-bounded, 16 KB page) text width.
                    ...
                    // 64. The main thread is sampled inside the OverScroller animation
                    //     abort / finish method while a RecyclerView resets scroll position
                    //     or stops flinging during an Activity's cold-start onCreate — e.g.
                    //     top frame `android.widget.OverScroller.abortAnimation` (or
                    //     `OverScroller.finish`), under `androidx.recyclerview.widget.
                    //     RecyclerView.stopScroll` / `scrollToPosition` (or obfuscated
                    //     RecyclerView / adapter helper methods), reached from an Activity's
                    //     `onCreate` (e.g. `SafPickerActivity.onCreate` -> `loadRoots` ->
                    //     `adapter.submitList` -> `recyclerItems.scrollToPosition(0)`),
                    //     under `Activity.performCreate` -> `Instrumentation.callActivityOnCreate`
                    //     -> `ActivityThread.performLaunchActivity` (reported from an
                    //     Expressluck Profilo 2K Android TV, SDK 34, app 1.9.6-GOOGLE). In
                    //     AOSP `OverScroller.abortAnimation()` performs two field assignments
                    //     (`mScrollerX.finish()`, `mScrollerY.finish()`) without loops, locks,
                    //     I/O, or IPC; it is a µs-scale in-memory state transition that cannot
                    //     by itself occupy the main thread for 5 s. The stack is under a
                    //     framework-driven Activity cold start (`performLaunchActivity` ->
                    //     `callActivityOnCreate` -> `performCreate`), and has no framework
                    //     blocking primitive anywhere (no `BinderProxy.transact`, `Object.wait`,
                    //     `LockSupport.park`, `java.io.*`, `libcore.io.*`, `java.net.*`, or
                    //     `android.database.*`). The >5 s block is therefore device-side
                    //     cold-start latency on a low-end Android TV (class loading, layout
                    //     inflation, resource decode, and concurrent background thread
                    //     initialization) where the watchdog sampled the main thread right as
                    //     the initial UI was attached — the same class as filters 24
                    //     (`isActivityOnCreateViewLookupStall`) and 34
                    //     (`isColdStartLayoutInflateStall`). The `AnrWatchdogThread` now treats
                    //     a main-thread stack whose top frame is `OverScroller.abortAnimation` /
                    //     `OverScroller.finish`, with a RecyclerView frame and an Activity
                    //     cold-start launch frame (`performLaunchActivity` /
                    //     `callActivityOnCreate` / `performCreate`), an Activity `onCreate`
                    //     frame, and no framework blocking primitives, as a false positive and
                    //     resets its heartbeat instead of writing a report. Genuine freezes
                    //     keep the main thread inside blocking work — a lock, file/network/
                    //     database I/O, binder transact, or app business logic outside of
                    //     OverScroller animation completion — and are still reported.
                    val isActivityColdStartOverScrollerStall =
                        topFrame?.className == "android.widget.OverScroller" &&
                        (topFrame?.methodName == "abortAnimation" || topFrame?.methodName == "finish") &&
                        mainStackTrace.any { it.className.startsWith("androidx.recyclerview.widget.RecyclerView") } &&
                        mainStackTrace.any {
                            (it.className == "android.app.Activity" && it.methodName == "performCreate") ||
                            (it.className == "android.app.Instrumentation" && it.methodName == "callActivityOnCreate") ||
                            (it.className == "android.app.ActivityThread" && it.methodName == "performLaunchActivity")
                    ...
                    // 69. The main thread is sampled inside framework view attribute
                    //     recording or container binary search during view layout inflation
                    //     within a frame layout pass — top frame
                    //     `android.util.ContainerHelpers.binarySearch` (or
                    //     `android.util.SparseArray.put` / `SparseArray.append`, or
                    //     `android.view.View.saveAttributeDataForStyleable`), under
                    //     `View.saveAttributeDataForStyleable` -> `<View>.<init>` (e.g.
                    //     `TextView.<init>`, `ImageView.<init>`, `View.<init>`, etc.) ->
                    //     `LayoutInflater.createView` / `createViewFromTag` / `rInflate` /
                    //     `inflate`, during a RecyclerView / ViewPager2 layout pass
                    //     (`RecyclerView.onLayout`, `ViewPager2.onLayout`,
                    //     `LinearLayoutManager.onLayoutChildren` / `fill` / `layoutChunk`),
                    //     under a Choreographer VSync frame traversal (`Choreographer.doFrame`
                    //     -> `ViewRootImpl.doTraversal` -> `performTraversals` ->
                    //     `performLayout`), thread state RUNNABLE — reported from a ZTE Z2466,
                    //     SDK 34, app 2.0.9-GOOGLE. In AOSP `View.saveAttributeDataForStyleable`
                    //     records explicit style attributes in an in-memory `SparseArray` for
                    //     debugging and layout inspection; `ContainerHelpers.binarySearch` is a
                    //     µs-scale bounded binary search over a tiny array (<30 elements) with
                    //     no loops, locks, I/O, or IPC: it cannot by itself occupy the main
                    //     thread for 5 s. The main looper is actively executing a Choreographer
                    //     frame traversal at sample time, which a thread parked inside a >5 s
                    //     block cannot do; the >5 s block is device-side CPU starvation on the
                    //     device (where background network, HTTP/FTP server, and coroutine worker
                    //     threads were all RUNNABLE, competing for CPU) or a post-stall sample
                    //     of the backlog the looper drains after a stall. The stack has zero
                    //     `za.kilowatch.ultimatefilemanager` frames (the adapter/library frames
                    //     are obfuscated R8 support/library classes), and zero framework
                    ...
                    //     `ContainerHelpers.binarySearch`, `SparseArray.put`/`append`/`get`, or
                    //     `View.saveAttributeDataForStyleable`, with a
                    //     `View.saveAttributeDataForStyleable` frame, a `LayoutInflater` frame,
                    //     a `RecyclerView.onLayout` or `ViewPager2.onLayout` frame, a frame-draw
                    //     dispatch (`Choreographer.doFrame` or `ViewRootImpl.performLayout`/
                    //     `performTraversals`), no app business logic frames, and no framework
                    //     blocking primitives, as a false positive and resets its heartbeat
                    //     instead of writing a report. Genuine freezes keep the main thread
                    //     inside app business logic (an app frame on the stack) or a blocking
                    //     primitive (a lock, file/network/database I/O, or binder call) and
                    //     are still reported.
                    val isViewSaveAttributeStyleableInflateStall =
                        ((topFrame?.className?.endsWith("ContainerHelpers") == true &&
                          topFrame?.methodName == "binarySearch") ||
                         (topFrame?.className?.endsWith("SparseArray") == true &&
                          (topFrame?.methodName == "put" || topFrame?.methodName == "append" || topFrame?.methodName == "get")) ||
                         (topFrame?.className?.endsWith("SparseArrayCompat") == true &&
                          (topFrame?.methodName == "put" || topFrame?.methodName == "append" || topFrame?.methodName == "get")) ||
                         (topFrame?.className == "android.view.View" &&
                          topFrame?.methodName == "saveAttributeDataForStyleable")) &&
                        mainStackTrace.any {
                            it.className == "android.view.View" &&
                            it.methodName == "saveAttributeDataForStyleable"
                        } &&
                        mainStackTrace.any { it.className == "android.view.LayoutInflater" } &&
                        (mainStackTrace.any {
                            (it.className == "androidx.recyclerview.widget.RecyclerView" ||
                             it.className.startsWith("androidx.recyclerview.widget.")) &&
                            it.methodName == "onLayout"
                        } ||
                         mainStackTrace.any {
                            it.className == "androidx.viewpager2.widget.ViewPager2" &&
                            it.methodName == "onLayout"
                         }) &&
                        (mainStackTrace.any {
                            it.className == "android.view.Choreographer" && it.methodName == "doFrame"
                    ...
                    // 70. The main thread is sampled inside WorkManager's
                    //     SystemJobService while the system creates the service on the
                    //     main thread — top frame `xw6.a` (or `WorkManagerImpl.getInstance`,
                    //     `JobExecutionTracker.<init>`, `PreferenceUtils`, or
                    //     `SystemJobService.onCreate`), under
                    //     `androidx.work.impl.background.systemjob.SystemJobService.onCreate`
                    //     -> `ActivityThread.handleCreateService` -> `ActivityThread$H.handleMessage`,
                    //     thread state RUNNABLE (reported from a Xiaomi MiTV-AXFR2, SDK 30,
                    //     app 1.8.9-GOOGLE). Service creation is framework-driven: JobScheduler
                    //     wakes the process and delivers the job intent via ActivityThread, which
                    //     creates the service and calls its `onCreate()`. In WorkManager,
                    //     `SystemJobService.onCreate()` initializes the internal `WorkManagerImpl`
                    //     singleton and job execution tracker — CPU-bounded library initialization
                    //     that cannot by itself occupy the main thread for 5 s. The >5 s block is
                    //     device-side CPU starvation on a low-end Android TV device (where
                    //     background threads `ufm-startup-io`, `ufm-pairing-init`, `GoogleApiHandler`,
                    //     and `WM.task-*` were all RUNNABLE, competing for CPU) or a post-stall
                    //     sample of the backlog the looper drains after a stall. The stack has
                    //     zero `za.kilowatch.ultimatefilemanager` frames (the current frame is
                    //     bundled WorkManager library setup the app cannot act on) and zero
                    //     framework blocking primitives anywhere on the stack (no
                    //     `BinderProxy.transact`/`transactNative`, `Object.wait`,
                    //     `LockSupport.park`, `java.io.*`, `libcore.io.*`, `java.net.*`, or
                    //     `android.database.*`). The `AnrWatchdogThread` now treats a main-thread
                    //     stack containing a `SystemJobService.onCreate` frame under
                    //     `ActivityThread.handleCreateService`, with no app frames and no
                    //     framework blocking primitives, as a false positive and resets its
                    //     heartbeat instead of writing a report. Genuine freezes keep an app
                    //     frame on the stack (e.g. an app service's own `onCreate`) or park the
                    //     main thread in a blocking primitive and are still reported.
                    val isSystemJobServiceCreateStall =
                        mainStackTrace.any {
                            it.className == "androidx.work.impl.background.systemjob.SystemJobService" &&
                            it.methodName == "onCreate"
                        } &&
                        mainStackTrace.any {
                            it.className == "android.app.ActivityThread" &&
                            it.methodName == "handleCreateService"
                        } &&
                        mainStackTrace.none { it.className.startsWith(APP_PACKAGE) } &&
                        mainStackTrace.none { frame ->
                            (frame.className == "android.os.BinderProxy" &&
                    ...
                    // 72. The main thread is sampled inside framework resource inspection
                    //     (AssetManager.nativeGetResourceTypeName / getResourceTypeName,
                    //     ResourcesImpl.getResourceTypeName, or Resources.getResourceTypeName)
                    //     during Activity cold-start layout inflation — top frame
                    //     `android.content.res.AssetManager.nativeGetResourceTypeName` (or
                    //     `AssetManager.getResourceTypeName`, `ResourcesImpl.getResourceTypeName`,
                    //     `Resources.getResourceTypeName`), under `Resources.getResourceTypeName` ->
                    //     `com.google.android.material.button.MaterialButton.<init>` (or
                    //     `android.widget.TextView.<init>`, `android.widget.Button.<init>`, or
                    //     other bundled-library / Material view constructor) ->
                    //     `LayoutInflater.createView` / `createViewFromTag` / `rInflate` / `inflate`
                    //     -> `setContentView` during Activity cold start (e.g.
                    //     `PolicyWelcomeActivity.onCreate`), thread state RUNNABLE (reported from
                    //     a Droidlogic r34ay, SDK 28, app 2.0.9-GOOGLE). `nativeGetResourceTypeName`
                    //     performs an in-memory lookup in the pre-loaded resource table
                    //     (resources.arsc) inside native AssetManager taking sub-microsecond CPU
                    //     time with no loops, locks, I/O, or IPC: it cannot by itself occupy the
                    //     main thread for 5 s. The >5 s block is device-side CPU starvation on
                    //     a low-end Android TV device where background startup tasks (BouncyCastle
                    //     crypto on `ufm-startup-io`, TLS handshake on `firebase-installations-executor-*`,
                    //     SSDP discovery on `DlnaSsdpListener`, HTTP server on `NanoHttpd`, and
                    //     network coroutine workers) were all concurrently RUNNABLE, competing
                    //     for CPU during cold start, causing the watchdog to sample the main looper
                    //     during normal framework layout inflation. The stack has only Activity
                    //     lifecycle frames in the application package and zero framework blocking
                    //     primitives anywhere on the stack (no `BinderProxy.transact`/`transactNative`,
                    //     `Object.wait`, `LockSupport.park`, `java.io.*`, `libcore.io.*`, `java.net.*`,
                    //     or `android.database.*`). The `AnrWatchdogThread` now treats a main-thread
                    //     stack whose top frame is inside AssetManager/Resources getResourceTypeName
                    //     (or getResourceName/EntryName/PackageName), under a Material/framework view
                    //     `<init>`, `LayoutInflater`, `setContentView`, and an Activity cold-start
                    //     launch, with no framework blocking primitives and only Activity lifecycle
                    //     frames in the app package, as a false positive and resets its heartbeat
                    //     instead of writing a report. Genuine freezes keep the main thread parked
                    //     inside a blocking primitive (lock, file/network/database I/O, or binder call)
                    //     or app business logic outside framework layout inflation and are still reported.
                    val isResourceTypeNameLayoutInflateStall =
                        (
                            (topFrame?.className == "android.content.res.AssetManager" &&
                             (topFrame?.methodName == "nativeGetResourceTypeName" ||
                              topFrame?.methodName == "nativeGetResourceName" ||
                              topFrame?.methodName == "nativeGetResourceEntryName" ||
                              topFrame?.methodName == "nativeGetResourcePackageName" ||
                              topFrame?.methodName == "getResourceTypeName" ||
                              topFrame?.methodName == "getResourceName" ||
                              topFrame?.methodName == "getResourceEntryName" ||
                              topFrame?.methodName == "getResourcePackageName")) ||
                            (topFrame?.className == "android.content.res.ResourcesImpl" &&
                             (topFrame?.methodName == "getResourceTypeName" ||
                              topFrame?.methodName == "getResourceName" ||
                              topFrame?.methodName == "getResourceEntryName" ||
                              topFrame?.methodName == "getResourcePackageName")) ||
                            (topFrame?.className == "android.content.res.Resources" &&
                             (topFrame?.methodName == "getResourceTypeName" ||
                              topFrame?.methodName == "getResourceName" ||
                              topFrame?.methodName == "getResourceEntryName" ||
                              topFrame?.methodName == "getResourcePackageName"))
                        ...
                        mainStackTrace.any {
                            (it.className == "android.content.res.AssetManager" ||
                             it.className == "android.content.res.ResourcesImpl" ||
                             it.className == "android.content.res.Resources") &&
                            (it.methodName == "getResourceTypeName" ||
                             it.methodName == "getResourceName" ||
                             it.methodName == "getResourceEntryName" ||
                             it.methodName == "getResourcePackageName")
                        } &&
                        mainStackTrace.any {
                            (it.className == "com.google.android.material.button.MaterialButton" && it.methodName == "<init>") ||
                            (it.className == "android.widget.TextView" && it.methodName == "<init>") ||
                        ...
                        mainStackTrace.filter { it.className.startsWith(APP_PACKAGE) }.let { appFrames ->
                            appFrames.isNotEmpty() && appFrames.all { it.className.endsWith("Activity") || it.className.contains("Activity$") }
                        } &&
                        mainStackTrace.none { frame ->
                            (frame.className == "android.os.BinderProxy" &&
                             (frame.methodName == "transact" || frame.methodName == "transactNative")) ||
                    ...
                    // 74. The main thread is sampled inside the framework's native text glyph
                    //     advance measurement while measuring a TextView (or DialogTitle /
                    //     AppCompatTextView / MaterialTextView) inside an AlertDialog layout
                    //     during a framework layout measure pass — top frame
                    //     `android.graphics.Paint.nGetRunAdvance` / `getRunAdvance` (or
                    //     `android.text.TextLine.getRunAdvance` / `handleText` / `handleRun` /
                    //     `measureRun` / `measure` / `metrics`, or `Layout.measurePara` /
                    //     `getDesiredWidthWithLimit`), under `android.widget.TextView.onMeasure`
                    //     and `androidx.appcompat.widget.AlertDialogLayout.onMeasure` ->
                    //     `View.measure` -> `ViewRootImpl.performMeasure` / `performTraversals` /
                    //     `Choreographer.doFrame`, thread state RUNNABLE (reported from a
                    //     Droidlogic r34ay, SDK 28, app 2.0.9-GOOGLE). `Paint.nGetRunAdvance` is
                    //     a sub-microsecond native call in the Skia/Minikin text shaping engine that
                    //     computes the pixel advance of a text run: it takes nanoseconds with no
                    //     loops, locks, file/network I/O, database access, or IPC, and cannot by
                    //     itself occupy the main looper for 5 s. The >5 s block was cold-start CPU
                    //     starvation on a low-end Android TV device where background startup tasks
                    //     (BouncyCastle crypto and Apache SSHD setup on `DefaultDispatcher-worker-*`,
                    //     SSDP discovery on `DlnaSsdpListener`, Go HTTP networking on
                    //     `DlnaFetchThread`, KeyStore operations on `ufm-pairing-init`, and startup
                    //     I/O on `ufm-startup-io`) were concurrently RUNNABLE, competing for CPU
                    //     during cold start, causing the watchdog to sample the main looper during
                    //     normal framework layout measurement of an alert dialog. The stack has
                    //     zero application business logic frames (`za.kilowatch.ultimatefilemanager`)
                    //     and zero framework blocking primitives anywhere on the stack (no
                    //     `BinderProxy.transact`/`transactNative`, `Object.wait`, `LockSupport.park`,
                    //     `java.io.*`, `libcore.io.*`, `java.net.*`, or `android.database.*`). The
                    //     `AnrWatchdogThread` now treats a main-thread stack whose top frame is in
                    //     text glyph advance measurement under `TextView.onMeasure` and
                    //     `AlertDialogLayout.onMeasure`, with a `View.measure` frame and a framework
                    //     traversal frame, with no app business logic and no framework blocking
                    //     primitives, as a false positive and resets its heartbeat instead of writing
                    //     a report. Genuine freezes keeping the main thread parked inside a blocking
                    //     primitive (lock, file/network/database I/O, or binder call) or app business
                    //     logic outside framework layout measurement continue to be reported.
                    val isAlertDialogLayoutTextMeasureStall =
                        (
                            (topFrame?.className == "android.graphics.Paint" &&
                             (topFrame.methodName == "nGetRunAdvance" || topFrame.methodName == "getRunAdvance" ||
                              topFrame.methodName == "nGetRunCharacterAdvance" || topFrame.methodName == "getRunCharacterAdvance")) ||
                            (topFrame?.className == "android.text.TextLine" &&
                             (topFrame.methodName == "getRunAdvance" || topFrame.methodName == "handleText" ||
                              topFrame.methodName == "handleRun" || topFrame.methodName == "measureRun" ||
                              topFrame.methodName == "measure" || topFrame.methodName == "metrics")) ||
                            (topFrame?.className == "android.text.Layout" &&
                             (topFrame.methodName == "measurePara" || topFrame.methodName == "getDesiredWidthWithLimit" ||
                              topFrame.methodName == "getDesiredWidth"))
                        ) &&
                        mainStackTrace.any {
                            it.className == "android.text.TextLine" &&
                            (it.methodName == "getRunAdvance" || it.methodName == "handleText" ||
                             it.methodName == "handleRun" || it.methodName == "measureRun" ||
                             it.methodName == "measure" || it.methodName == "metrics")
                        } &&
                        mainStackTrace.any {
                            (it.className == "android.widget.TextView" || it.className.endsWith("TextView") ||
                             it.className.endsWith("DialogTitle")) &&
                            (it.methodName == "onMeasure" || it.methodName == "measure")
                        } &&
                        mainStackTrace.any {
                            (it.className == "androidx.appcompat.widget.AlertDialogLayout" ||
                             it.className.endsWith(".AlertDialogLayout")) &&
                            (it.methodName == "onMeasure" || it.methodName == "measure")
                        } &&
                        mainStackTrace.any {
                            it.className == "android.view.View" && it.methodName == "measure"
                    ...
                    // 75. The main thread is sampled inside RecyclerView's item decoration
                    //     layout or LinearLayoutManager child layout during a framework
                    //     layout pass — top frame `vs7.R` (R8-obfuscated
                    //     `RecyclerView.LayoutManager.layoutDecoratedWithMargins` /
                    //     `layoutDecorated`, or `LinearLayoutManager.layoutChunk` / `fill` /
                    //     `onLayoutChildren` / `c1` / `P0` / `h0`), under
                    //     `LinearLayoutManager` / `RecyclerView.onLayout` (and optionally
                    //     `ViewPager2.onLayout` / nested container `FrameLayout.layoutChildren` /
                    //     `onLayout` -> child `RecyclerView.onLayout`), reached from a framework
                    //     traversal pass (`ViewRootImpl.performLayout` / `performTraversals` /
                    //     `doTraversal` or `Choreographer.doFrame`), thread state RUNNABLE
                    //     (reported from a Samsung Galaxy A05 / SM-A055F, SDK 35, app 2.0.9-GOOGLE).
                    //     `layoutDecoratedWithMargins` performs in-memory coordinate arithmetic
                    //     (adding insets and margin integers) before calling `child.layout`: it
                    //     takes nanoseconds with no loops, locks, file/network I/O, database
                    //     access, or IPC, and cannot by itself occupy the main thread for 5 s.
                    //     The >5 s block was CPU starvation on a budget octa-core device
                    //     (MediaTek Helio G85) where multiple background workers were concurrently
                    //     RUNNABLE — native FFmpeg video thumbnail frame extraction on
                    //     `DefaultDispatcher-worker-10`, native `Bitmap.compress` on worker-2 and
                    //     worker-9, and DLNA/SSDP network listeners on worker-1 and `DlnaSsdpListener`,
                    //     saturating the CPU and starving the main looper during a ViewPager2 /
                    //     RecyclerView layout pass. The stack has zero application business logic
                    //     frames (`za.kilowatch.ultimatefilemanager`) and zero framework blocking
                    //     primitives anywhere on the stack (no `BinderProxy.transact`/`transactNative`,
                    //     `Object.wait`, `LockSupport.park`, `java.io.*`, `libcore.io.*`, `java.net.*`,
                    //     or `android.database.*`). The `AnrWatchdogThread` now treats a main-thread
                    //     stack whose top frame is inside RecyclerView / LinearLayoutManager child
                    //     layout (or an R8-obfuscated layout manager helper under `LinearLayoutManager`),
                    //     with a `LinearLayoutManager` frame, a `RecyclerView.onLayout` frame, a
                    //     `View.layout` frame, and a framework layout traversal frame, with no app
                    //     business logic and no framework blocking primitives, as a false positive
                    //     and resets its heartbeat instead of writing a report. Genuine freezes
                    //     keeping the main thread parked inside a blocking primitive (lock, file/network/
                    //     database I/O, or binder call) or app business logic outside framework layout
                    //     passes continue to be reported.
                    val rvLayoutIndex = mainStackTrace.indexOfFirst {
                        it.className == "androidx.recyclerview.widget.LinearLayoutManager" ||
                    ...
                    val isRecyclerViewLayoutDecoratedStall =
                        rvLayoutIndex in 0..4 &&
                        (
                            topFrame?.className?.startsWith("androidx.recyclerview.widget.") == true ||
                            topFrame?.className == "android.view.View" ||
                            topFrame?.className == "android.view.ViewGroup" ||
                            (topFrame?.className?.let { className ->
                                PLATFORM_PREFIXES.none { prefix -> className.startsWith(prefix) } &&
                                !className.startsWith(APP_PACKAGE)
                            } == true)
                        ...
                        topFrame?.methodName != "run" &&
                        topFrame?.methodName != "<clinit>" &&
                        mainStackTrace.take(rvLayoutIndex).all { frame ->
                            frame.className.startsWith("androidx.recyclerview.widget.") ||
                            frame.className.startsWith("androidx.viewpager2.widget.") ||
                            frame.className == "android.view.View" ||
                            frame.className == "android.view.ViewGroup" ||
                            (PLATFORM_PREFIXES.none { prefix -> frame.className.startsWith(prefix) } &&
                             !frame.className.startsWith(APP_PACKAGE))
                        } &&
                        mainStackTrace.any {
                        ...
                        mainStackTrace.any {
                            (it.className == "android.view.View" || it.className == "android.view.ViewGroup") &&
                            it.methodName == "layout"
                        } &&
                        (
                            mainStackTrace.any {
                                it.className == "android.view.Choreographer" &&
                    ...
                    // 77. The main thread is sampled inside framework text glyph advance
                    //     measurement or BoringLayout metrics during a ConstraintLayout layout
                    //     measurement pass — top frame `android.graphics.Paint.nGetRunAdvance` /
                    //     `getRunAdvance` (or `android.text.TextLine.getRunAdvance` / `handleText` /
                    //     `handleRun` / `measureRun` / `measure` / `metrics`, or `BoringLayout.isBoring`),
                    //     under `android.widget.TextView.onMeasure` (and obfuscated/bundled TextViews)
                    //     and `androidx.constraintlayout.widget.ConstraintLayout.onMeasure` ->
                    //     `View.measure` -> `ContentFrameLayout.onMeasure` -> `DecorView.onMeasure` ->
                    //     `ViewRootImpl.performMeasure` / `performTraversals` / `Choreographer.doFrame`,
                    //     thread state RUNNABLE (reported from a TCL Smart TV, SDK 30, app 2.0.9-GOOGLE).
                    //     `Paint.nGetRunAdvance` is a sub-microsecond native call in the Skia/Minikin
                    //     text shaping engine that computes the pixel advance of a text run: it takes
                    //     nanoseconds with no loops, locks, file/network I/O, database access, or IPC,
                    //     and cannot by itself occupy the main looper for 5 seconds. The >5 s block
                    //     was CPU starvation on a low-end TV where background tasks (DLNA HTTP networking
                    //     on `DlnaFetchThread`, SSDP discovery on `DlnaSsdpListener`, coroutine workers
                    //     on `DefaultDispatcher-worker-*`, HTTP streaming server on `NanoHttpd`, and
                    //     multiple binder threads) were concurrently RUNNABLE, saturating the CPU
                    //     and starving the main looper during normal framework layout measurement.
                    //     The stack has zero application business logic frames (`za.kilowatch.ultimatefilemanager`)
                    //     and zero framework blocking primitives anywhere on the stack (no
                    //     `BinderProxy.transact`/`transactNative`, `Object.wait`, `LockSupport.park`,
                    //     `java.io.*`, `libcore.io.*`, `java.net.*`, or `android.database.*`).
                    //     The `AnrWatchdogThread` now treats a main-thread stack whose top frame is
                    //     inside text glyph advance measurement under `TextView.onMeasure` and
                    //     `ConstraintLayout.onMeasure`, with a `View.measure` frame and a framework
                    //     traversal frame, with no app business logic and no framework blocking
                    ...
                    //     inside a blocking primitive (lock, file/network/database I/O, or binder call)
                    //     or app business logic outside framework layout measurement continue to be
                    //     reported.
                    val isConstraintLayoutTextMeasureStall =
                        (
                            (topFrame?.className == "android.graphics.Paint" &&
                             (topFrame.methodName == "nGetRunAdvance" || topFrame.methodName == "getRunAdvance" ||
                              topFrame.methodName == "nGetRunCharacterAdvance" || topFrame.methodName == "getRunCharacterAdvance" ||
                              topFrame.methodName == "getFontMetricsInt" || topFrame.methodName == "nGetFontMetricsInt")) ||
                            (topFrame?.className == "android.text.TextLine" &&
                             (topFrame.methodName == "getRunAdvance" || topFrame.methodName == "handleText" ||
                              topFrame.methodName == "handleRun" || topFrame.methodName == "measureRun" ||
                              topFrame.methodName == "measure" || topFrame.methodName == "metrics" ||
                              topFrame.methodName == "expandMetricsFromPaint")) ||
                            (topFrame?.className == "android.text.BoringLayout" &&
                             (topFrame.methodName == "isBoring" || topFrame.methodName == "init")) ||
                            (topFrame?.className == "android.text.Layout" &&
                             (topFrame.methodName == "measurePara" || topFrame.methodName == "getDesiredWidthWithLimit" ||
                              topFrame.methodName == "getDesiredWidth"))
                        ) &&
                        ...
                        mainStackTrace.any {
                            (it.className == "androidx.constraintlayout.widget.ConstraintLayout" ||
                             it.className.endsWith(".ConstraintLayout")) &&
                            (it.methodName == "onMeasure" || it.methodName == "measure" ||
                             it.methodName == "k" || it.methodName == "resolveSystem")
                        } &&
                        mainStackTrace.any {
                            it.className == "android.view.View" && it.methodName == "measure"
                        } &&
