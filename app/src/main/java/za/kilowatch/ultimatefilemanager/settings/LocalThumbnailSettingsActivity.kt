...
/**
 * Local Media Thumbnail Settings activity.
 ...
 */
class LocalThumbnailSettingsActivity : AppCompatActivity() {

    private var isTv = false
    private lateinit var cacheManager: LocalThumbnailCacheManager
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cacheManager = LocalThumbnailCacheManager(this)
        isTv = DeviceUtils.isTvDevice(this)

        setContentView(if (isTv) R.layout.activity_local_thumbnails_tv else R.layout.activity_local_thumbnails)
        ...
        folderPickerLauncher = registerForActivityResult(
            ...
            if (result.resultCode == RESULT_OK) {
                ...
                if (path != null) {
                    ...
                    ThumbnailPreferenceManager.setCachePath(this, path)
                    updateUI()
                }
            }
        }
    ...
    private fun setupViews() {
        ...
        switchEnable.setOnCheckedChangeListener { _, isChecked ->
            ThumbnailPreferenceManager.setEnabled(this, isChecked)
        }
        ...
        cardEnable.setOnClickListener {
            ...
            ThumbnailPreferenceManager.setEnabled(this, newValue)
        ...
        val cardClearCache = findViewById<View?>(R.id.cardClearCache)
        if (isTv && cardClearCache != null) {
        ...
        if (isTv) {
            ...
            val settingsGroup = findViewById<View?>(R.id.cardSettingsGroup)
            if (settingsGroup is MaterialCardView) setupTvCardFocus(settingsGroup)
            if (cardClearCache is MaterialCardView) setupTvCardFocus(cardClearCache)
        }
    }

    fun updateUI() {
        switchEnable.isChecked = ThumbnailPreferenceManager.isEnabled(this)

        val currentPath = ThumbnailPreferenceManager.getCachePath(this)
        txtCacheFolder.text = getFriendlyPathName(currentPath)

        val limitMb = ThumbnailPreferenceManager.getCacheLimitMb(this)
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            val count = cacheManager.getCachedCount()
            val formattedSize = if (currentSizeMb >= 1024) {
                String.format(Locale.getDefault(), "%.2f GB", currentSizeMb.toDouble() / 1024.0)
            } else {
                "$currentSizeMb MB"
            }
            withContext(Dispatchers.Main) {
                txtStats.text = getString(R.string.lt_stats_size, formattedSize, limitMb, count)
            }
        }
    }
    ...
    private fun showStoragePickerGuideDialog() {
        val layoutRes = if (isTv) R.layout.dialog_local_cache_folder_guide_tv else R.layout.dialog_local_cache_folder_guide
        ...
        val defaultPath = File(externalCacheDir, "local_thumbnails").absolutePath
        ...
        btnResetDefault.setOnClickListener {
            ThumbnailPreferenceManager.setCachePath(this, "")
            updateUI()
            dialog.dismiss()
        }
    ...
    private fun showStoragePicker() {
        folderPickerLauncher.launch(
            Intent(this, za.kilowatch.ultimatefilemanager.storage.StorageBrowserActivity::class.java).apply {
                putExtra(za.kilowatch.ultimatefilemanager.storage.StorageBrowserActivity.EXTRA_LOCAL_CACHE_PICKER, true)
            }
        )
    }
    ...
    private fun showLimitDialog() {
        ...
        val currentMb = ThumbnailPreferenceManager.getCacheLimitMb(this)
        ...
        optCustom.setOnClickListener {
            dialog.dismiss()
            if (isTv) {
                val intent = Intent(this, NetworkThumbnailCustomLimitActivity::class.java).apply {
                    putExtra(NetworkThumbnailCustomLimitActivity.EXTRA_MODE, NetworkThumbnailCustomLimitActivity.MODE_LOCAL)
                }
                startActivity(intent)
            } else {
                val frag = CacheLimitDialogFragment.newInstance(CacheLimitDialogFragment.MODE_LOCAL)
    ...
    private fun setupTvCardFocus(card: MaterialCardView) {
        ...
        val isClearCard = (card.id == R.id.cardClearCache || card.id == R.id.btnClearCache)
    ...
    private fun setChildTextColorsTwo(view: View, primary: Int, secondary: Int, error: Int, isErrorCard: Boolean) {
        if (view is TextView) {
            if (isErrorCard && (view.id == R.id.txtClearTitle || view.text.toString().contains("Clear", true))) {
                view.setTextColor(error)
            } else if (view.id == R.id.txtCacheFolder || view.id == R.id.txtCacheLimit || view.id == R.id.txtStats) {
