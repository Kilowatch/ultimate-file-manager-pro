...
/**
 * Settings screen for configuring the auto-hide timeout of media player
 * controls (play/pause, seek bar, top bar, etc.).
 *
 * **Mobile**: a SeekBar (0–9) maps to 1 s – 10 s; changes persist immediately.
 * **TV**: left/right cards decrement/increment the step; a Save card persists.
 */
class ControlsTimeoutActivity : AppCompatActivity() {

    private var isTv = false
    private var currentStep = ControlsTimeoutManager.DEFAULT_STEP
    ...
    companion object {
        fun start(context: Context) =
            context.startActivity(Intent(context, ControlsTimeoutActivity::class.java))
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (isTv) {
            setContentView(R.layout.activity_controls_timeout_tv)
        } else {
            setContentView(R.layout.activity_controls_timeout)
        }
        ...
        currentStep = ControlsTimeoutManager.loadStep(this)
    ...
    private fun setupMobile() {
        ...
        seekBar?.max = ControlsTimeoutManager.STEP_COUNT - 1
        ...
        seekBar?.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(sb: SeekBar, progress: Int, fromUser: Boolean) {
                ...
                ControlsTimeoutManager.saveStep(this@ControlsTimeoutActivity, currentStep)
            }
    ...
    private fun setupTv() {
        ...
        btnIncrease?.setOnClickListener {
            if (currentStep < ControlsTimeoutManager.STEP_COUNT - 1) { currentStep++; onTvStepChanged() }
        }
        btnSave?.setOnClickListener {
            ControlsTimeoutManager.saveStep(this, currentStep)
            finish()
        }
    ...
    private fun buildStepDots() {
        ...
        for (i in 0 until ControlsTimeoutManager.STEP_COUNT) {
            val dot = View(this)
    ...
    private fun updateValueDisplay() {
        txtDurationValue.text = ControlsTimeoutManager.formatStep(currentStep)
    }
