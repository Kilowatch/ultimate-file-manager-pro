...
class FileBrowserFragment : Fragment() {
    ...
    private val safTreeLauncher = registerForActivityResult(
        ...
        if (result.resultCode == android.app.Activity.RESULT_OK) {
            ...
            context?.let { android.widget.Toast.makeText(it, R.string.protected_folder_saf_denied, android.widget.Toast.LENGTH_SHORT).show() }
        }
    }
    ...
    private fun setupViews(view: View) {
        ...
        btnChecksum = view.findViewById(R.id.btnChecksum)
        ...
        if (isTv) {
            ...
            recyclerFiles.setOnKeyListener { _, keyCode, event ->
                if (keyCode in listOf(KeyEvent.KEYCODE_DPAD_DOWN, KeyEvent.KEYCODE_DPAD_UP, KeyEvent.KEYCODE_DPAD_LEFT, KeyEvent.KEYCODE_DPAD_RIGHT)) {
                    when (event.action) {
        ...
        fabTools?.setOnClickListener {
            ...
            // 4a. Open With
            if (count == 1 && selected.first().isFile && pm.isIconEnabled(context, pm.KEY_OPEN_WITH)) {
                list.add(FileToolsBottomSheet.ActionItem("open_with", getString(R.string.toolbar_open_with), R.drawable.ic_apps, "toolbar_open_with") {
                    FileViewerRouter.showOpenWithDialog(requireActivity(), selected.first(), isNetwork = false)
                })
            }

            // 4b. Open As
            if (count == 1 && selected.first().isFile && pm.isIconEnabled(context, pm.KEY_OPEN_AS)) {
            ...
            if (isSingleImage && !DeviceUtils.isTvDevice(requireContext())) {
            ...
            if (isSingleAudio && !DeviceUtils.isTvDevice(requireContext())) {
                ...
                if (pm.isIconEnabled(context, pm.KEY_SET_RINGTONE)) {
                    list.add(FileToolsBottomSheet.ActionItem("set_ringtone", getString(R.string.action_set_ringtone), R.drawable.ic_ringtone, "toolbar_set_ringtone") {
                        za.kilowatch.ultimatefilemanager.util.RingtoneHelper.showConfirmDialog(
                            ...
                            if (!za.kilowatch.ultimatefilemanager.util.RingtoneHelper.canWriteSettings(requireContext())) {
                                android.widget.Toast.makeText(requireContext(), R.string.toast_sound_permission_required, android.widget.Toast.LENGTH_LONG).show()
                                za.kilowatch.ultimatefilemanager.util.RingtoneHelper.requestWriteSettings(requireContext())
                ...
                if (ext in listOf("sha256", "sha512", "sha1", "md5", "sfv", "crc")) {
                    list.add(FileToolsBottomSheet.ActionItem("verify_checksum_manifest", getString(R.string.action_verify_checksum_file), R.drawable.ic_checksum, "toolbar_checksum") {
                        ...
                        za.kilowatch.ultimatefilemanager.checksum.ChecksumManifestVerifyDialogFragment.newInstance(
                            ...
                            sel.parentFile
                        ).show(parentFragmentManager, za.kilowatch.ultimatefilemanager.checksum.ChecksumManifestVerifyDialogFragment.TAG)
                    })
                }
            }
    ...
    private fun setupTvFocus(view: View) {
        ...
        wireTvIconBtn(view.findViewById(R.id.btnChecksum)) { }
        ...
        when (actionId) {
            ...
            pm.ACTION_SET_RINGTONE -> {
                if (count == 1 && selected.first().isFile) {
                    val audioFile = selected.first()
                    za.kilowatch.ultimatefilemanager.util.RingtoneHelper.showConfirmDialog(
                        requireContext(), audioFile.name, android.media.RingtoneManager.TYPE_RINGTONE
                    ) {
                        if (!za.kilowatch.ultimatefilemanager.util.RingtoneHelper.canWriteSettings(requireContext())) {
                            android.widget.Toast.makeText(requireContext(), R.string.toast_sound_permission_required, android.widget.Toast.LENGTH_LONG).show()
                            za.kilowatch.ultimatefilemanager.util.RingtoneHelper.requestWriteSettings(requireContext())
                        ...
                        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
                            val success = za.kilowatch.ultimatefilemanager.util.RingtoneHelper.setAsSystemSound(
                                requireContext(), audioFile, android.media.RingtoneManager.TYPE_RINGTONE
                            )
                            withContext(Dispatchers.Main) {
                                fileAdapter.exitSelectionMode()
                                val msg = if (success) getString(R.string.toast_ringtone_set_success) else getString(R.string.toast_sound_set_failed)
            ...
            pm.ACTION_SET_NOTIFICATION -> {
                if (count == 1 && selected.first().isFile) {
                    val audioFile = selected.first()
                    za.kilowatch.ultimatefilemanager.util.RingtoneHelper.showConfirmDialog(
                        requireContext(), audioFile.name, android.media.RingtoneManager.TYPE_NOTIFICATION
                    ) {
                        if (!za.kilowatch.ultimatefilemanager.util.RingtoneHelper.canWriteSettings(requireContext())) {
                            android.widget.Toast.makeText(requireContext(), R.string.toast_sound_permission_required, android.widget.Toast.LENGTH_LONG).show()
                            za.kilowatch.ultimatefilemanager.util.RingtoneHelper.requestWriteSettings(requireContext())
                        ...
                        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
                            val success = za.kilowatch.ultimatefilemanager.util.RingtoneHelper.setAsSystemSound(
                                requireContext(), audioFile, android.media.RingtoneManager.TYPE_NOTIFICATION
                            )
                            withContext(Dispatchers.Main) {
                                fileAdapter.exitSelectionMode()
                                val msg = if (success) getString(R.string.toast_notification_set_success) else getString(R.string.toast_sound_set_failed)
            ...
            pm.ACTION_SET_ALARM -> {
                if (count == 1 && selected.first().isFile) {
                    val audioFile = selected.first()
                    za.kilowatch.ultimatefilemanager.util.RingtoneHelper.showConfirmDialog(
                        requireContext(), audioFile.name, android.media.RingtoneManager.TYPE_ALARM
                    ) {
                        if (!za.kilowatch.ultimatefilemanager.util.RingtoneHelper.canWriteSettings(requireContext())) {
                            android.widget.Toast.makeText(requireContext(), R.string.toast_sound_permission_required, android.widget.Toast.LENGTH_LONG).show()
                            za.kilowatch.ultimatefilemanager.util.RingtoneHelper.requestWriteSettings(requireContext())
                        ...
                        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
                            val success = za.kilowatch.ultimatefilemanager.util.RingtoneHelper.setAsSystemSound(
                                requireContext(), audioFile, android.media.RingtoneManager.TYPE_ALARM
                            )
                            withContext(Dispatchers.Main) {
                                fileAdapter.exitSelectionMode()
                                val msg = if (success) getString(R.string.toast_alarm_set_success) else getString(R.string.toast_sound_set_failed)
    ...
    private fun updateSelectionBar(count: Int) {
        ...
        if (showSelection) {
            ...
            btnChecksum?.visibility = if (showActions && selectedFiles.any { !it.isDirectory } && pm.isIconEnabled(context, pm.KEY_CHECKSUM)) View.VISIBLE else View.GONE
    ...
    private fun submitAdapterList(sorted: List<File>, showAllAsIndexed: Boolean?, hiddenPaths: Set<String>) {
        ...
        val restoreScroll = {
            ...
            } else if (restoredFolderState != null && !isTv) {
                recyclerFiles.post {
                    (recyclerFiles.layoutManager as? androidx.recyclerview.widget.LinearLayoutManager)
        ...
        if (isTv) {
            val requestFocus = shouldRestoreFocus || arguments?.getBoolean(ARG_REQUEST_INITIAL_FOCUS, false) == true || (isNavigatingFolder && restoredFolderState != null)
            ...
            if (requestFocus) {
                recyclerFiles.post {
                    var focusPos = 0
                    val exitedDir = lastExitedDir ?: (restoredFolderState?.targetChildPath?.let { File(it) })
                    if (exitedDir != null) {
                        val index = fileAdapter.findPosition(exitedDir.absolutePath)
                        if (index != -1) {
                            focusPos = index
                        } else if (restoredFolderState != null) {
                            focusPos = restoredFolderState.position.coerceIn(0, (fileAdapter.itemCount - 1).coerceAtLeast(0))
                        }
                        lastExitedDir = null
                    } else if (restoredFolderState != null) {
                        focusPos = restoredFolderState.position.coerceIn(0, (fileAdapter.itemCount - 1).coerceAtLeast(0))
    ...
    private fun showGoToPathDialog() {
        ...
        za.kilowatch.ultimatefilemanager.ui.UfmDialogHelper.showGoToPath(ctx, initial) { inputPath ->
