...
class FileBrowserActivity : AppCompatActivity(), VolumeEjectHost {
    ...
    companion object {
        ...
        /** When true, browsing root filesystem (/) */
        const val EXTRA_IS_ROOT_STORAGE = "extra_is_root_storage"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val rawMountPath = intent.getStringExtra(EXTRA_MOUNT_PATH)
        isRootStorage = intent.getBooleanExtra(EXTRA_IS_ROOT_STORAGE, false) ||
                        intent.getStringExtra(EXTRA_STORAGE_TYPE).equals("ROOT", ignoreCase = true) ||
                        rawMountPath == "/"
        ...
        if (!isRootStorage && (rootPath.isEmpty() || (!isRootProtected && !isRootSaf && !File(rootPath).exists()))) {
            ...
            storageLabel = intent.getStringExtra(EXTRA_STORAGE_LABEL) ?: if (isRootStorage) getString(R.string.root_storage_title) else getString(R.string.storage)
            val (resolvedSid, resolvedStype, _) = za.kilowatch.ultimatefilemanager.indexing.IndexingRepository.resolveStorageForPath(rootPath)
        ...
        if (!initialPath.isNullOrEmpty()) {
            ...
            val isInitRoot = isRootStorage || cleanInit == "/" || za.kilowatch.ultimatefilemanager.storage.RootShellWrapper.isRootPath(cleanInit)
            val initExists = if (isInitRoot) {
                za.kilowatch.ultimatefilemanager.storage.RootShellWrapper.exists(cleanInit)
            ...
            if (initExists && (isInitRoot || cleanInit.startsWith(rootPath))) {
                currentDir = if (isInitRoot) {
                    val pName = cleanInit.substringAfterLast("/")
                    val pParent = cleanInit.substringBeforeLast("/", "")
                    za.kilowatch.ultimatefilemanager.storage.RootFile(pParent, pName, true)
                } else if (isInitProtected && ShizukuShellWrapper.canUseShizukuForPath(cleanInit)) {
                    val pName = cleanInit.substringAfterLast("/")
                    val pParent = cleanInit.substringBeforeLast("/", "")
                    ShizukuFile(pParent, pName, true)
                } else if (isInitSaf) {
    ...
    private fun showConfirmLocationPickerLocalFolderDialog() {
        val path = currentDir.absolutePath
        val isSafExplicit = za.kilowatch.ultimatefilemanager.storage.SafTreeManager.isSafPath(path)
        val matchingSafLoc = if (isSafExplicit) {
            val locId = path.removePrefix("saf://").substringBefore('/')
            ...
            za.kilowatch.ultimatefilemanager.storage.SafLocationRepository.getLocations(this).firstOrNull { loc ->
                ...
                disp.isNotEmpty() && (path == disp || path.startsWith("$disp/"))
            }
        }
        val isSaf = isSafExplicit || matchingSafLoc != null ||
        ...
        val resolvedUri = when {
            ...
            matchingSafLoc != null -> {
                ...
                val rel = path.removePrefix(baseDisp).trimStart('/')
                if (rel.isEmpty()) "saf://${matchingSafLoc.id}" else "saf://${matchingSafLoc.id}/$rel"
            }
            isSaf -> "saf://$path"
            else -> "file://$path"
        }
        ...
        val folderName = when {
            ...
            matchingSafLoc != null -> matchingSafLoc.displayName
    ...
    private fun setupViews() {
        ...
        fabTools?.setOnClickListener {
            ...
            // 4a. Open With
            if (count == 1 && selected.first().isFile && pm.isIconEnabled(this, pm.KEY_OPEN_WITH)) {
                list.add(FileToolsBottomSheet.ActionItem("open_with", getString(R.string.toolbar_open_with), R.drawable.ic_apps, "toolbar_open_with") {
                    FileViewerRouter.showOpenWithDialog(this, selected.first(), isNetwork = false)
                })
            }

            // 4b. Open As
            if (count == 1 && selected.first().isFile && pm.isIconEnabled(this, pm.KEY_OPEN_AS)) {
                list.add(FileToolsBottomSheet.ActionItem("open_as", getString(R.string.toolbar_open_as), R.drawable.ic_apps, "toolbar_open_as") {
                    OpenAsBottomSheet.newInstance(selected.first().absolutePath, selected.first().name, false)
            ...
            if (count == 1 && selected.first().isFile && FileViewerRouter.isVideo(selected.first().extension) &&
                ...
                list.add(FileToolsBottomSheet.ActionItem("extract_subtitles", getString(R.string.toolbar_extract_subtitles), R.drawable.ic_subtitles, "toolbar_extract_subtitles") {
                    extractSubtitlesFromVideo(selected.first())
                })
            }
            ...
            if (count == 1 && selected.first().isFile && FileViewerRouter.isVideo(selected.first().extension) &&
                ...
                list.add(FileToolsBottomSheet.ActionItem("extract_audio", getString(R.string.toolbar_extract_audio), R.drawable.ic_audio_track, "toolbar_extract_audio") {
                    extractAudioFromVideo(selected.first())
                })
            }
            ...
            if (count == 1 && selected.first().isFile && FileViewerRouter.isVideo(selected.first().extension) &&
                ...
                list.add(FileToolsBottomSheet.ActionItem("convert_to_mp4", getString(R.string.toolbar_convert_to_mp4), R.drawable.ic_convert_video, "toolbar_convert_to_mp4") {
                    convertVideoToMp4(selected.first())
                })
            }
            ...
            if (allAudio && !DeviceUtils.isTvDevice(this) && pm.isIconEnabled(this, pm.KEY_MUSIC_TAGGER)) {
    ...
    private fun handleQuickActionClick(actionId: String) {
        ...
        when (actionId) {
            pm.ACTION_DELETE -> showDeleteConfirmation()
            ...
            "protect", pm.ACTION_PROTECT_UNPROTECT -> {
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    for (file in selected) {
                        za.kilowatch.ultimatefilemanager.settings.ProtectedFilesManager.setProtected(this@FileBrowserActivity, file.absolutePath, protected = targetProtect)
                    }
                    withContext(Dispatchers.Main) {
                        fileAdapter.exitSelectionMode()
                        loadDirectory(currentDir)
            ...
            "pin", pm.ACTION_PIN_UNPIN -> {
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    for (file in selected) {
                        za.kilowatch.ultimatefilemanager.settings.PinnedFilesManager.setPinned(this@FileBrowserActivity, file.absolutePath, pinned = targetPin)
                    }
                    withContext(Dispatchers.Main) {
                        fileAdapter.exitSelectionMode()
                        loadDirectory(currentDir)
            ...
            pm.ACTION_SET_HOME_WALLPAPER -> {
                if (count == 1 && selected.first().isFile) {
                    ...
                    za.kilowatch.ultimatefilemanager.util.WallpaperHelper.showConfirmDialog(
                        ...
                        lifecycleScope.launch(Dispatchers.IO) {
                            val success = za.kilowatch.ultimatefilemanager.util.WallpaperHelper.setWallpaper(
                                this@FileBrowserActivity, imageFile, android.app.WallpaperManager.FLAG_SYSTEM
                            )
                            withContext(Dispatchers.Main) {
                                fileAdapter.exitSelectionMode()
                                val msg = if (success) getString(R.string.toast_wallpaper_set_home_success) else getString(R.string.toast_wallpaper_set_failed)
            ...
            pm.ACTION_SET_LOCK_WALLPAPER -> {
                if (count == 1 && selected.first().isFile) {
                    ...
                    za.kilowatch.ultimatefilemanager.util.WallpaperHelper.showConfirmDialog(
                        ...
                        lifecycleScope.launch(Dispatchers.IO) {
                            val success = za.kilowatch.ultimatefilemanager.util.WallpaperHelper.setWallpaper(
                                this@FileBrowserActivity, imageFile, android.app.WallpaperManager.FLAG_LOCK
                            )
                            withContext(Dispatchers.Main) {
                                fileAdapter.exitSelectionMode()
                                val msg = if (success) getString(R.string.toast_wallpaper_set_lock_success) else getString(R.string.toast_wallpaper_set_failed)
    ...
    private fun extractSubtitlesFromVideo(file: File) {
        lifecycleScope.launch(Dispatchers.IO) {
            val info = za.kilowatch.ultimatefilemanager.media.FFmpegMediaHelper.getMediaInfo(file)
            val subStreams = info?.subtitleStreams ?: emptyList()
            withContext(Dispatchers.Main) {
                if (subStreams.isEmpty()) {
                    Toast.makeText(this@FileBrowserActivity, R.string.no_subtitles_found_in_video, Toast.LENGTH_SHORT).show()
                    return@withContext
                }
                ...
                if (subStreams.size == 1) {
                    ...
                    lifecycleScope.launch(Dispatchers.IO) {
                        ...
                        withContext(Dispatchers.Main) {
                            ...
                            if (result != null) {
                                fileAdapter.exitSelectionMode()
                                Toast.makeText(this@FileBrowserActivity, getString(R.string.subtitles_extracted_success) + "\n" + result.name, Toast.LENGTH_LONG).show()
                                loadDirectory(currentDir)
                            } else {
                                Toast.makeText(this@FileBrowserActivity, R.string.error_generic, Toast.LENGTH_SHORT).show()
                            }
                        }
                    }
                } else {
                            ...
                            if (extractAll) {
                                ...
                                withContext(Dispatchers.Main) {
                                    ...
                                    if (results.isNotEmpty()) {
                                        Toast.makeText(this@FileBrowserActivity, getString(R.string.subtitles_extracted_multiple_success, results.size), Toast.LENGTH_LONG).show()
                                    } else {
                                        Toast.makeText(this@FileBrowserActivity, R.string.error_generic, Toast.LENGTH_SHORT).show()
                                    }
                                    loadDirectory(currentDir)
    ...
    private fun extractAudioFromVideo(file: File) {
        lifecycleScope.launch(Dispatchers.IO) {
            val info = za.kilowatch.ultimatefilemanager.media.FFmpegMediaHelper.getMediaInfo(file)
            val audioStreams = info?.audioStreams ?: emptyList()
            withContext(Dispatchers.Main) {
                if (audioStreams.isEmpty()) {
                    Toast.makeText(this@FileBrowserActivity, R.string.no_audio_found_in_video, Toast.LENGTH_SHORT).show()
                    return@withContext
                }
                    ...
                    lifecycleScope.launch(Dispatchers.IO) {
                        if (extractAll) {
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                fileAdapter.exitSelectionMode()
                                if (results.isNotEmpty()) {
                                    Toast.makeText(this@FileBrowserActivity, getString(R.string.audio_extracted_multiple_success, results.size), Toast.LENGTH_LONG).show()
                                } else {
                                    Toast.makeText(this@FileBrowserActivity, R.string.error_generic, Toast.LENGTH_SHORT).show()
                                }
                                loadDirectory(currentDir)
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                fileAdapter.exitSelectionMode()
                                if (result != null) {
                                    Toast.makeText(this@FileBrowserActivity, getString(R.string.audio_extracted_success) + "\n" + result.name, Toast.LENGTH_LONG).show()
                                } else {
                                    Toast.makeText(this@FileBrowserActivity, R.string.error_generic, Toast.LENGTH_SHORT).show()
                                }
                                loadDirectory(currentDir)
            ...
            .setPositiveButton(R.string.action_convert_to_mp4) { _, _ ->
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    withContext(Dispatchers.Main) {
                        ...
                        if (result != null) {
                            fileAdapter.exitSelectionMode()
                            Toast.makeText(this@FileBrowserActivity, getString(R.string.convert_to_mp4_success, result.name), Toast.LENGTH_LONG).show()
                            loadDirectory(currentDir)
                        } else {
                            Toast.makeText(this@FileBrowserActivity, R.string.convert_to_mp4_failed, Toast.LENGTH_SHORT).show()
                        }
                    }
                }
            }
                ...
                try {
                    ...
                    for (file in filesToDelete) {
                        ...
                        val success = if (isRoot) {
                            za.kilowatch.ultimatefilemanager.storage.RootShellWrapper.delete(file.absolutePath)
        ...
        btnSaveRename?.setOnClickListener {
            ...
            if (newName.isNotEmpty() && newName != file.name) {
                ...
                val newFile = if (isRoot) {
                    za.kilowatch.ultimatefilemanager.storage.RootFile(file.parent ?: "/", newName, file.isDirectory)
                } else {
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    val success = if (isRoot) {
                        za.kilowatch.ultimatefilemanager.storage.RootShellWrapper.move(file.absolutePath, newFile.absolutePath)
                    } else if (isSaf) {
    ...
    private fun createTextFile(baseName: String) {
        ...
        val isRoot = isRootStorage || currentDir is za.kilowatch.ultimatefilemanager.storage.RootFile || za.kilowatch.ultimatefilemanager.storage.RootShellWrapper.isRootPath(currentDir.absolutePath)
        val fileToCreate = if (isSaf) {
            za.kilowatch.ultimatefilemanager.storage.SafFile(currentDir.absolutePath, baseName, false)
        } else if (isRoot) {
            var targetFile = za.kilowatch.ultimatefilemanager.storage.RootFile(currentDir.absolutePath, baseName, false)
            if (targetFile.exists()) {
                val nameWithoutExt = targetFile.nameWithoutExtension
                val ext = targetFile.extension
                var counter = 2
                while (targetFile.exists()) {
                    targetFile = za.kilowatch.ultimatefilemanager.storage.RootFile(currentDir.absolutePath, "$nameWithoutExt ($counter).$ext", false)
                    counter++
                }
            }
            targetFile
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                val created = if (isSaf) {
                    ...
                    za.kilowatch.ultimatefilemanager.storage.RootShellWrapper.createFile(fileToCreate.absolutePath)
                } else {
                    fileToCreate.createNewFile()
                }
                if (created) {
                    // Index the new file (skip for root partitions)
                    if (!isRoot) {
                        try {
        ...
        dialogView.findViewById<View>(R.id.btnCreate)?.setOnClickListener {
            ...
            if (name.isEmpty()) {
                ...
                val newDir = if (isRoot) {
                    za.kilowatch.ultimatefilemanager.storage.RootFile(currentDir.absolutePath, name, true)
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    val exists = if (isRoot) {
                        za.kilowatch.ultimatefilemanager.storage.RootShellWrapper.exists(newDir.absolutePath)
                    } else if (za.kilowatch.ultimatefilemanager.storage.ShizukuShellWrapper.canUseShizukuForPath(currentDir.absolutePath)) {
                    ...
                    val created = if (exists) {
                        ...
                        za.kilowatch.ultimatefilemanager.storage.RootShellWrapper.mkdir(newDir.absolutePath)
                    } else if (za.kilowatch.ultimatefilemanager.storage.ShizukuShellWrapper.canUseShizukuForPath(currentDir.absolutePath)) {
    ...
    private fun isRestrictedStorageRoot(dir: File, sessionRoot: String): Boolean {
        ...
        if (isAppSpecificPath(dir.absolutePath)) return false
        if (dir.absolutePath.startsWith("saf://") || za.kilowatch.ultimatefilemanager.storage.SafTreeManager.isSafPath(dir.absolutePath)) return false
        if (dir.absolutePath != sessionRoot) return false

        val canList = try {
            val list = dir.listFiles()
            list != null && dir.canRead()
        } catch (_: Exception) {
            false
        }
        ...
        return dir.absolutePath == internalRoot ||
               dir.absolutePath == "/storage/emulated/0" ||
               dir.absolutePath == "/sdcard" ||
    ...
    private fun loadDirectory(directory: File, preserveSelection: Boolean = false) {
        ...
        val isRootDir = isRootStorage || targetDir is za.kilowatch.ultimatefilemanager.storage.RootFile || za.kilowatch.ultimatefilemanager.storage.RootShellWrapper.isRootPath(targetDir.absolutePath)
        if (isRootDir) {
            folderFlowJob = lifecycleScope.launch(Dispatchers.IO) {
                val showHidden = za.kilowatch.ultimatefilemanager.settings.HiddenFilesManager.isShowHiddenFilesEnabled
                val hiddenPaths = za.kilowatch.ultimatefilemanager.settings.HiddenFilesDatabase.getInstance(applicationContext).hiddenFileDao().getAllPaths().toSet()
    ...
    private fun sortAndFilterFiles(files: List<File>): List<File> {
        ...
        val fileComparator = SortFilterPreferenceManager.getFileComparator(state, this, directoriesFirst = true)
    ...
    private fun performExtract(archives: List<File>, customDestFolder: File? = null, isSelectFolderMode: Boolean) {
        ...
        val progressDialog = ArchiveProgressDialog(this@FileBrowserActivity)
    ...
    private fun updateEmptyState(isEmpty: Boolean) {
        if (isEmpty) {
            ...
            val isSaf = za.kilowatch.ultimatefilemanager.storage.SafTreeManager.isSafPath(currentDir.absolutePath) ||
                    za.kilowatch.ultimatefilemanager.storage.SafTreeManager.hasTreePermissionForPath(this, currentDir.absolutePath) ||
                    currentDir is za.kilowatch.ultimatefilemanager.storage.SafFile ||
                    currentDir.absolutePath.startsWith("saf://")
            val isRestrictedRoot = isRestrictedStorageRoot(currentDir, rootPath)
            ...
            if (isRestrictedRoot) {
                ...
                btnEnableElevated?.visibility = View.GONE
                ...
                btnEnableElevated?.visibility = View.VISIBLE
    ...
    private fun updateBreadcrumbs() {
        ...
        val isRoot = isRootStorage || rootPath == "/"
        val list = mutableListOf<Pair<String, File?>>()
        list.add(Pair("Home", null))
        val rootFileObj: File = if (isRoot) za.kilowatch.ultimatefilemanager.storage.RootFile("", "")
                                else if (isRootSaf) SafFile(rootPath, true)
                                else File(rootPath)
        list.add(Pair(storageLabel, rootFileObj))
        
        if (currentDir.absolutePath != rootPath && (isRoot || currentDir.absolutePath.startsWith(rootPath))) {
            val relativePath = if (rootPath == "/") currentDir.absolutePath.removePrefix("/")
                               else currentDir.absolutePath.substring(rootPath.length).removePrefix("/")
            if (relativePath.isNotEmpty()) {
                val parts = relativePath.split("/")
                var currentAccumulated: File = rootFileObj
                for (part in parts) {
                    if (part.isNotEmpty()) {
                        currentAccumulated = if (isRoot) za.kilowatch.ultimatefilemanager.storage.RootFile(currentAccumulated.absolutePath, part, true)
                                             else if (isRootSaf) SafFile(currentAccumulated.absolutePath, part, true)
                                             else File(currentAccumulated, part)
