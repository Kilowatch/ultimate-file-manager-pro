...
class FilePropertiesBottomSheet : BottomSheetDialogFragment() {
    ...
    private fun resolveFileType(name: String, isDirectory: Boolean): Pair<String, String> {
        ...
        return when (ext) {
            ...
            "png", "apng" -> Pair("${ext.uppercase()} image (.${ext})", "${ext.uppercase()} Image (.${ext})")
            ...
            "hdr", "exr" -> Pair("${ext.uppercase()} HDR image (.${ext})", "${ext.uppercase()} High Dynamic Range Image (.${ext})")
            "mpo", "jps", "pns" -> Pair("3D stereoscopic image (.${ext})", "3D Stereoscopic Image (.${ext})")
            "tiff", "tif" -> Pair("TIFF image (.${ext})", "Tagged Image File Format (.${ext})")
            "ico", "cur", "ani" -> Pair("Icon / Cursor image (.${ext})", "Windows Icon / Cursor (.${ext})")
            "tga", "targa", "dds" -> Pair("${ext.uppercase()} texture image (.${ext})", "${ext.uppercase()} Texture Image (.${ext})")
            "psd", "psb" -> Pair("Photoshop document (.${ext})", "Adobe Photoshop Document (.${ext})")
            "ai" -> Pair("Illustrator artwork (.ai)", "Adobe Illustrator Artwork (.ai)")
            "xcf" -> Pair("GIMP image (.xcf)", "GIMP Image Document (.xcf)")
            "kra" -> Pair("Krita document (.kra)", "Krita Painting Document (.kra)")
            "clip" -> Pair("Clip Studio document (.clip)", "Clip Studio Paint Document (.clip)")
            "wbmp", "pcx", "pbm", "pgm", "ppm", "pnm" -> Pair("${ext.uppercase()} bitmap (.${ext})", "${ext.uppercase()} Bitmap Image (.${ext})")
            "wmf", "emf" -> Pair("Windows Metafile (.${ext})", "Windows Metafile Vector Graphics (.${ext})")

            // Camera RAW
            "cr2", "cr3", "crw" -> Pair("Canon RAW image (.${ext})", "Canon Camera RAW Image (.${ext})")
            "nef", "nrw" -> Pair("Nikon RAW image (.${ext})", "Nikon Camera RAW Image (.${ext})")
            "arw", "srf", "sr2" -> Pair("Sony RAW image (.${ext})", "Sony Camera RAW Image (.${ext})")
            "raf" -> Pair("Fujifilm RAW image (.raf)", "Fujifilm Camera RAW Image (.raf)")
            "rw2" -> Pair("Panasonic RAW image (.rw2)", "Panasonic Lumix RAW Image (.rw2)")
            "orf" -> Pair("Olympus RAW image (.orf)", "Olympus Camera RAW Image (.orf)")
            "pef", "ptx" -> Pair("Pentax RAW image (.${ext})", "Pentax Camera RAW Image (.${ext})")
            "dng" -> Pair("Adobe DNG image (.dng)", "Digital Negative RAW Image (.dng)")
            "srw" -> Pair("Samsung RAW image (.srw)", "Samsung Camera RAW Image (.srw)")
            "x3f" -> Pair("Sigma RAW image (.x3f)", "Sigma Foveon RAW Image (.x3f)")
            "erf" -> Pair("Epson RAW image (.erf)", "Epson Camera RAW Image (.erf)")
            "kdc", "dcr", "k25" -> Pair("Kodak RAW image (.${ext})", "Kodak Camera RAW Image (.${ext})")
            "mrw" -> Pair("Minolta RAW image (.mrw)", "Minolta Camera RAW Image (.mrw)")
            ...
            "mov", "qt" -> Pair("QuickTime video (.${ext})", "QuickTime Movie (.${ext})")
            "mjpeg", "mjpg", "mjp" -> Pair("Motion JPEG video (.${ext})", "Motion JPEG Video (.${ext})")
            "m4v" -> Pair("Apple video (.m4v)", "Apple Video Container (.m4v)")
            "webm" -> Pair("WebM video (.webm)", "WebM Video (.webm)")
            "wmv", "asf", "wm" -> Pair("Windows Media video (.${ext})", "Windows Media Video (.${ext})")
            "flv", "f4v" -> Pair("Flash video (.${ext})", "Flash Video (.${ext})")
            "3gp", "3g2", "3gp2" -> Pair("3GPP video (.${ext})", "3GPP Multimedia Video (.${ext})")
            "ts", "m2ts", "mts", "m2t", "tp", "trp" -> Pair("Transport Stream video (.${ext})", "MPEG Transport Stream Video (.${ext})")
            "vob", "evo" -> Pair("DVD video object (.${ext})", "DVD Video Object (.${ext})")
            "mpg", "mpeg", "mpe", "m1v", "m2v", "mpv" -> Pair("MPEG video (.${ext})", "MPEG Video (.${ext})")
            "ogv", "ogm" -> Pair("Ogg video (.${ext})", "Ogg Media Video (.${ext})")
            "rm", "rmvb" -> Pair("RealMedia video (.${ext})", "RealMedia Video (.${ext})")
            "mxf" -> Pair("MXF broadcast video (.mxf)", "Material Exchange Format Video (.mxf)")
            "dv" -> Pair("Digital video (.dv)", "Digital Video Recording (.dv)")
            "divx", "xvid" -> Pair("${ext.uppercase()} video (.${ext})", "${ext.uppercase()} Video (.${ext})")
            "mk3d" -> Pair("Matroska 3D video (.mk3d)", "Matroska 3D Stereoscopic Video (.mk3d)")
